  ...
  return (
    <div className='min-h-screen flex bg-white'>
      ...
      <aside className='hidden lg:block sticky top-[65px] h-[calc(100vh-65px)] w-80 bg-white border-r border-gray-100 z-10 overflow-y-auto'>
        <TableOfContents chapters={book.chapters} currentSlug={chapterSlug} bookSlug={bookSlug} />
      </aside>
      ...
      <main className='flex-1 min-w-0'>
        <div className='w-full bg-slate-50 py-12'>
          <div className='container mx-auto px-4 max-w-3xl'>
            <div className='flex flex-col items-center text-center space-y-6'>
              <Link href={`/books/${bookSlug}`} className='flex items-center text-gray-700 hover:text-gray-900'>
                <BookOpen className='h-5 w-5 mr-2' />
                <span className='font-medium'>{book.title}</span>
              </Link>
              <h1 className='text-3xl md:text-4xl font-bold tracking-tight'>{chapter.title}</h1>
        ...
        <article className='container mx-auto px-4 py-12 max-w-3xl'>
          <div className='prose prose-lg max-w-none' dangerouslySetInnerHTML={{ __html: chapter.content }} />
        </article>
        ...
        <div className='container mx-auto px-4 py-8 max-w-3xl border-t border-gray-100'>
          <div className='flex flex-col sm:flex-row justify-between items-center gap-4'>
            {prevChapter && (
              <Link href={`/books/${bookSlug}/${prevChapter?.slug || chapter.slug}`}>
                <Button variant='outline' className='w-full sm:w-auto px-24! py-12 text-lg'>
            ...
            {nextChapter && (
              <Link href={`/books/${bookSlug}/${nextChapter?.slug || chapter.slug}`}>
                <Button variant='outline' className='w-full sm:w-auto px-24! py-12 text-lg'>
                  次の章
