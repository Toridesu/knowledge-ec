      ...
      <div className='grid lg:grid-cols-[400px_1fr]'>
        <div className='relative w-full'>
          <div className='relative aspect-[1/1.4] w-full'>
            <Image src={content?.coverImage || ''} alt='Featured content' fill priority className='object-cover' sizes='(max-width: 1024px) 100vw, 400px' />
          </div>
        </div>
        <div className='flex flex-col'>
          <CardContent className='grid flex-1 gap-4 p-6'>
            <div className='flex items-center gap-4'>
              ...
              <Badge className='bg-primary/10 text-primary hover:bg-primary/20'>New</Badge>
            </div>
            ...
            <div>
              ...
              <div className='space-y-2 text-gray-600'>
                ...
                {content?.chapters && content?.chapters.length > 5 && <div className='text-gray-500 mt-2'>（他 {content?.chapterCount - 5} タスク）</div>}
              </div>
            </div>
          </CardContent>
          <CardFooter className='flex items-center justify-between border-t p-6'>
            <div className='text-2xl font-bold flex items-center'>
              ...
              <span className='text-sm font-normal text-muted-foreground line-through mx-2'>￥{FEATURED_CONTENT.properPrice.toLocaleString()}</span>
              <span className='text-sm flex font-medium text-muted-foreground'>{FEATURED_CONTENT.discount}%OFF</span>
            </div>

            <Button size='lg' className='bg-gradient-to-r from-purple-500 to-blue-500 font-semibold'>
              今すぐ購入する
            </Button>
          </CardFooter>
        </div>
